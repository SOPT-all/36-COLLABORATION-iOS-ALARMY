...
final class DefaultTempService: TempServiceProtocol {
    ...
    func getTest() async throws -> String {
        do {
            let response: BaseReponse<TempModelResponse> = try await network.request(endpoint: .fetch)
    ...
    func postTest(id: Int) async throws -> String {
        do {
            ...
            let response: BaseReponse<TempModelResponse> = try await network.request(
                endpoint: .login,
                body: request
            )
